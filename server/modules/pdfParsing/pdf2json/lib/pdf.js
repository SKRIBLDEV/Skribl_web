...
var PDFJSClass = (function () {
    ...
    //[H] changed this function significantly in order to retrieve metadata useful for Skribl application 
    ...
    cls.prototype.parseMetaData = function() {
        ...
        //dictionnary to translate between XML-entities and properties for publicationMetadata objects
        var keysDict = {
            // dc: "Dublin Core", prism : "Publishing Requirements for Industry Standard Metadata"
            // -> XML-elements used for descriptive metadata
            title : "dc:title",
            authors : "dc:creator",
            subject : "dc:subject",
            description : "dc:description",
            dateOfPublication : "prism:publicationdate",
        ...
        // if there is no metadata found, the empty publicationMetadata object is passed
        if (metadata){
            for (var key in keysDict) {
                if (keysDict.hasOwnProperty(key)) {
                    if (metadata.has(keysDict[key])){
                        publicationMetadata[key] = metadata.get(keysDict[key]);
                    };
                };
            }; 
        
            if (publicationMetadata.authors){ // [H] convert to array (VERY UGLY)  : TO DO : make this work the right way (using the different XML entities collected)  
                var temp = publicationMetadata.authors;
                publicationMetadata.authors = [temp];
            };
        };

        self.emit("pdfjs_parseDataReady", {Transcoder: pkInfo._id, Agency:pdfTile, Id: publicationMetadata}); //pass standard metadata instead of pdfx data as Id property
    };
