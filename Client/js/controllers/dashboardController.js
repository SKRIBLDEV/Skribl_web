  ...
  //------------------------------------------------MANAGE PUBLICATIONS-------------------------------------------------//
    ...
    $scope.creatLib = function(libName){  
        ...
        createRequest.success(function(data, status, headers, config) {
            $scope.busy = false;
            toast("Library ".concat(libName).(" created."), 4000);
		});
   ...
   $scope.deleteLib = function(libName){  
        ...
        createRequest.success(function(data, status, headers, config) {
            $scope.busy = false;
            toast("Library ".concat(libName).(" deleted."), 4000);
		});
		createRequest.error(function(data, status, headers, config) {
            $scope.busy = false;
            toast("Failed to delete publication, try again later.", 4000);
    ...
    $scope.searchPublication = function(keyword,external){
        ...
        var url;
        if(external){url = serverApi.concat('publications?q=').concat(keyword).concat('&external=true');}
        else{url = serverApi.concat('publications?q=').concat(keyword);}
