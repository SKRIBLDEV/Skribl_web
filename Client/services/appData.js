 ...
 */
 ...
 webapp.service('appData', function() {
    ...
    this.removeResearchDomain = function(idx){
        if (idx > -1){
            self.data.currentMetaData.researchDomains.splice(idx, 1);
        } 
    }
    ...
    this.getMajorName = function(majorID){
        ...
        self.researchDomains.forEach(function(entry) {
            if (entry.major.ID == majorID){
                rname = entry.major.name;
            }
        });
        return rname;
    }

    this.getMinorName = function(majorID, minorID){
        var rname;
        self.researchDomains.forEach(function(entry) {
            if (entry.major.ID == majorID){
                entry.minor.forEach(function(entry){
                    if (entry.ID == minorID){
                        rname =  entry.name;
                    }
                })
